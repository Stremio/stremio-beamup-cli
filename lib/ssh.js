...
async function getSecret(opts, secret) {
	// ssh dokku@deployer.beamup.dev config:get 768c7b2546f2/hello NODE_ENV
	...
	const child = spawn('ssh', ['dokku@' + opts.host, 'config:get', githubUsernameHashed + '/' + opts.projectName, secret], {
		cwd: process.cwd(),
		env: Object.create(process.env),
		stdio: [process.stdin, process.stdout, process.stderr]
	})
