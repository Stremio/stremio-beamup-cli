...
async function project(opts) {
	...
	if (!usingSaved) {
		let config = { projectName: opts.projectName, lastCommit: opts.lastCommit || '' };
		if(host && host !== opts.host) config.host = host;
		opts.askDeploy = true
		await fs.writeFile(beamupProjectFile, JSON.stringify(config, null, 4))
...
async function saveLastCommit(opts) {
	if (opts.projectName) {
		...
		{
			var projectOpts = { projectName: opts.projectName, lastCommit: commitHash }
			if(opts.hostLocal) projectOpts.host = opts.hostLocal;
			await fs.writeFile(beamupProjectFile, JSON.stringify(projectOpts, null, 4))
