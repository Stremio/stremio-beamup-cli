...
async function project(opts) {
	...
	if (!opts.projectName)
		opts.projectName = await requestValue({
			...
			wrongRegex: /[^a-z0-9-]/,
			error: 'Error: Project name must begin with lowercase alphanumeric character, and cannot include uppercase characters, colons, or underscores',
		})

	if (!usingSaved) {
		opts.askDeploy = true
		await fs.writeFile(beamupProjectFile, JSON.stringify({ projectName: opts.projectName, lastCommit: opts.lastCommit || '' }, null, 4))
		...
		await ssh.addRemote(opts).catch(e => {
			console.log('An error occured while adding the Beam Up git remote, maybe the remote already exists?\n If you are trying to change the project name, please run "git remote remove beamup" and try again.')
		})
	}
	...
	async function loadFromFile(file) {
		...
		if (savedExists) {
			try {
				...
				return Promise.resolve(utils.sanitizeOpts(JSON.parse(savedData)))
			} catch(e) {
...
async function saveLastCommit(opts) {
	if (opts.projectName) {
		const commitHash = await lastCommit()

		if (commitHash && utils.isString(commitHash))
			await fs.writeFile(beamupProjectFile, JSON.stringify({ projectName: opts.projectName, lastCommit: commitHash }, null, 4))
	}
